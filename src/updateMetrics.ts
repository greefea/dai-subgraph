...
import { convertTokenToDecimal, getPoolTVL } from "./utils";
...
export function updateLiquidityPoolMetrics(event: ethereum.Event): void {
  ...
  let ERC20Token0 = ERC20.bind(Address.fromString(token0.id));
  let ERC20Token1 = ERC20.bind(Address.fromString(token1.id));
  let ERC20LPToken = ERC20.bind(Address.fromString(LPtoken));
  let token0Balance = BIGINT_ZERO;
  let token1Balance = BIGINT_ZERO;
  let token0result = ERC20Token0.try_balanceOf(
    Address.fromString(eventAddress)
  );
  let token1result = ERC20Token1.try_balanceOf(
    Address.fromString(eventAddress)
  );
  ...
  pool.totalValueLockedUSD = getPoolTVL(token0.id, token1.id, eventAddress);
  pool.inputTokenBalances = [token0Balance, token1Balance];
  pool._inputTokenBalancesAmount = [
    convertTokenToDecimal(token0Balance, token0.decimals),
    convertTokenToDecimal(token1Balance, token1.decimals),
  ];
  pool.save();
...
export function updateProtocolMetrics(
  ...
  if (user == null) {
    ...
    dailyProtocolSnapshot.cumulativeUniqueUsers =
      protocol.cumulativeUniqueUsers;
    hourlyProtocolSnapshot.cumulativeUniqueUsers =
      protocol.cumulativeUniqueUsers;
  }
...
export function updateProtcolFinancialsMetrics(event: ethereum.Event): void {
  ...
  let tvl = dailyFinancialsSnapshot.totalValueLockedUSD.plus(
...
export function updateVolumeAndFees(
  ...
  let supplyFee = BigDecimal.fromString("0.25").div(
    BigDecimal.fromString("100")
  );
  let protocolFee = BigDecimal.fromString("0.05").div(
    BigDecimal.fromString("100")
  );
  ...
  dailypoolSnapshot.dailyVolumeUSD = dailypoolSnapshot.dailyVolumeUSD.plus(
    trackedAmountUSD[2]
  );
  hourlypoolSnapshot.hourlyVolumeUSD = hourlypoolSnapshot.hourlyVolumeUSD.plus(
    trackedAmountUSD[2]
  );

  dailyFinancialsSnapshot.dailyVolumeUSD = dailyFinancialsSnapshot.dailyVolumeUSD.plus(
    trackedAmountUSD[2]
  );

  poolentity!.cumulativeVolumeUSD = poolentity!.cumulativeVolumeUSD.plus(
    trackedAmountUSD[2]
  );
  protocolentity!.cumulativeVolumeUSD = protocolentity!.cumulativeVolumeUSD.plus(
    trackedAmountUSD[2]
  );
  ...
  protocolentity!.cumulativeTotalRevenueUSD = protocolentity!.cumulativeTotalRevenueUSD.plus(
    tradingFeeAmountUSD
  );
  protocolentity!.cumulativeSupplySideRevenueUSD = protocolentity!.cumulativeSupplySideRevenueUSD.plus(
    supplyFeeAmountUSD
  );
  protocolentity!.cumulativeProtocolSideRevenueUSD = protocolentity!.cumulativeProtocolSideRevenueUSD.plus(
    protocolFeeAmountUSD
  );

  poolentity!.cumulativeTotalRevenueUSD = poolentity!.cumulativeTotalRevenueUSD.plus(
    tradingFeeAmountUSD
  );
  poolentity!.cumulativeSupplySideRevenueUSD = poolentity!.cumulativeSupplySideRevenueUSD.plus(
    supplyFeeAmountUSD
  );
  poolentity!.cumulativeProtocolSideRevenueUSD = poolentity!.cumulativeProtocolSideRevenueUSD.plus(
    protocolFeeAmountUSD
  );

  dailyFinancialsSnapshot.dailyTotalRevenueUSD = dailyFinancialsSnapshot.dailyTotalRevenueUSD.plus(
    tradingFeeAmountUSD
  );
  dailyFinancialsSnapshot.dailySupplySideRevenueUSD = dailyFinancialsSnapshot.dailySupplySideRevenueUSD.plus(
    supplyFeeAmountUSD
  );
  dailyFinancialsSnapshot.dailyProtocolSideRevenueUSD = dailyFinancialsSnapshot.dailyProtocolSideRevenueUSD.plus(
    protocolFeeAmountUSD
  );
  ...
  dailypoolSnapshot.dailyTotalRevenueUSD = dailypoolSnapshot.dailyTotalRevenueUSD.plus(
    tradingFeeAmountUSD
  );
  dailypoolSnapshot.dailySupplySideRevenueUSD = dailypoolSnapshot.dailySupplySideRevenueUSD.plus(
    supplyFeeAmountUSD
  );
  dailypoolSnapshot.dailyProtocolSideRevenueUSD = dailypoolSnapshot.dailyProtocolSideRevenueUSD.plus(
    protocolFeeAmountUSD
  );
  ...
  hourlypoolSnapshot.hourlyTotalRevenueUSD = hourlypoolSnapshot.hourlyTotalRevenueUSD.plus(
    tradingFeeAmountUSD
  );
  hourlypoolSnapshot.hourlySupplySideRevenueUSD = hourlypoolSnapshot.hourlySupplySideRevenueUSD.plus(
    supplyFeeAmountUSD
  );
  hourlypoolSnapshot.hourlyProtocolSideRevenueUSD = hourlypoolSnapshot.hourlyProtocolSideRevenueUSD.plus(
    protocolFeeAmountUSD
  );
