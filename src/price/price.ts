...
export function updateNativeTokenPrice(): Token {
  ...
  for (let i = 0; i < STABLE_ORACLE_POOLS.length; i++) {
    let pool = LiquidityPool.load(STABLE_ORACLE_POOLS[i]);
    if (!pool) continue;
    if (pool.inputTokens[0] == NATIVE_ADDRESS) {
      if (pool._inputTokenBalancesAmount![1] > stableAmount) {
        nativeAmount = pool._inputTokenBalancesAmount![0];
        stableAmount = pool._inputTokenBalancesAmount![1];
      }
    } else {
      if (pool._inputTokenBalancesAmount![0] > stableAmount) {
        nativeAmount = pool._inputTokenBalancesAmount![1];
        stableAmount = pool._inputTokenBalancesAmount![0];
      }
    }
  }
...
export function findUSDPriceToken(
  token: Token,
  nativeToken: Token
): BigDecimal {
    ...
    for (let i = 0; i < whiteList.length; ++i) {
      ...
      if (pool!.outputTokenSupply!.gt(BIGINT_ZERO)) {
        if (
          ...
          let whitelistToken = getOrCreateToken(
            Address.fromString(pool!.inputTokens[1])
          );
          let whitelistTokenLocked = pool._inputTokenBalancesAmount![1].times(
          ...
          if (whitelistTokenLocked.gt(largestLiquidityWhitelistTokens)) {
            ...
            priceSoFar = safeDiv(
              pool._inputTokenBalancesAmount![1],
              pool._inputTokenBalancesAmount![0]
        ...
        if (
          ...
          let whitelistTokenLocked = pool._inputTokenBalancesAmount![0].times(
            whitelistToken.lastPriceUSD!
          );
          if (whitelistTokenLocked.gt(largestLiquidityWhitelistTokens)) {
            largestLiquidityWhitelistTokens = whitelistTokenLocked;
  ...
  if (poolDeposits.valueInt < 5) {
    ...
    if (
      ...
      if (
        reserve0USD!
          .plus(reserve1USD)
          .lt(MINIMUM_LIQUIDITY_FOUR_HUNDRED_THOUSAND)
      ) {
        return [BIGDECIMAL_ZERO, BIGDECIMAL_ZERO, BIGDECIMAL_ZERO];
      }
    }
